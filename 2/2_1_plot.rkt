...
(struct submarine ([x : Integer] [y : Integer]) #:mutable)
...
(define (interpret commands [fn '()])
  (define sub (submarine 0 0))
  (for ([cmd commands])
    (match cmd
      [(up n) (set-submarine-y! sub (- (submarine-y sub) n))]
      [(down n) (set-submarine-y! sub (+ (submarine-y sub) n))]
      [(forward n) (set-submarine-x! sub (+ (submarine-x sub) n))])
    (when (not (null? fn))
      (fn sub)))
  sub)
...
(define resulting-submarine
  (call-with-input-file
    ...
    (lambda ([in : Input-Port])
      (interpret (in-commands (in-lines in)) push-submarine-position))))
